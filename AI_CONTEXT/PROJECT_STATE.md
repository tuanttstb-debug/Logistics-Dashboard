...
> **Δ 2026-07-25 (v0.3.0):** Hoàn tất **Phase A** (Route ×3 + Loại hàng ×2 + UpdateManual + POB trong `Transform.gs`; `report_` tách Full/POB) và **Phase B** (trang **Logistics record**: `report.js` lr*, `views.js`, `app.js` 2 chart, `?action=pob`). **QĐ-51** POB không vào Full của 3 trang cũ (áp cả `getMeta.totalUsd`=Full, `pobUsd` riêng). Test `test/run_tests.cjs` **49/49 PASS** + xác minh trình duyệt (EVD). Sửa `Setup.gs` chịu lỗi "cột đã nhập" (try/catch từng cột). **Chờ user** dán GS mới + chạy `rebuildFact()` + deploy New version.
