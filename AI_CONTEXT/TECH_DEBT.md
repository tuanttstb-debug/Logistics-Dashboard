...
| **TD-19** | **Code GAS mới chưa chạy trên Sheets thật** (Route/Loại hàng/POB + getMeta + `?action=pob`) | Web `?action=pob` báo lỗi tới khi **Deploy New version**; fact chưa có Route/Loại hàng/POB tới khi user chạy `rebuildFact` | User dán `backend/*.gs` → `rebuildFact()` → **Deploy New version** |
