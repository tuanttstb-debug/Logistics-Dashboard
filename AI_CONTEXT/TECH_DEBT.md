...
| **TD-12** | 🟡 **Web App "Anyone"** + URL trong repo → ai có link đọc được cost thật (shipper/consignee/số tiền) | Lộ dữ liệu tài chính | 🔧 **Đang xử lý (2026-08-06):** deployment-ID GAS đã **redact khỏi TOÀN lịch sử** (`env.js`→`REDACTED-ROTATED`). **User đang rotate** (redeploy URL mới) + đổi quyền khỏi "Anyone". Chờ URL mới để dán lại `env.js`. Còn: phân quyền GAS (TD-10) |
