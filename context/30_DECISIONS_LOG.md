...
| **QĐ-44** | **GAS tự dựng `fact_CostLines` TỪ raw (thay Power Query).** `backend/Transform.gs::rebuildFact()` — batch: đọc raw 10–19 + `22_Map_Cost`/`23_Map_ExchangeRate` + tháng ở `00_Config!B1`, tính rồi GHI ra tab **`40_FACT_CostLines`** (đổi tên từ `fact_CostLines` để khớp quy ước số như raw 10–19). Web vẫn CHỈ ĐỌC tab này. **v1 tăng dần:** courier (DHL/FedEx Exp/Imp) + Overhead; **lõi 4 trường** `Amount_USD`·`Standard Cost`·`Mode chuẩn`·`Import/Export` (Route/Loại hàng null). VVMV/Dolphin/EI + Route/Loại hàng bổ sung sau. **THAY một phần QĐ-43** ("Excel vẫn là engine"): Excel PQ nay là tham chiếu logic, không còn nguồn dán fact | User đảo nguồn để pipeline Sheets+GAS tự chủ. Chọn batch (không tính mỗi request — sheet 17 ~6500 dòng, rủi ro quota). Tăng dần để cô lập sai lệch; validate tổng khớp Excel (v1: 481 dòng/$12.940,87; đủ 7 nguồn: 1.480/$44.062) | ✅ |
